...
class SparkAPI_Core {
   ...
   */
	...
	function MakeAPICall($method, $service, $cache_time = 0, $params = array(), $post_data = null, $a_retry = false) {
		...
		global $debug;
		fwrite($debug, print_r($json,true));
	...
	function GetListings($params = array()) {
		return $this->return_all_results($this->MakeAPICall("GET", "listings", '2m', $params));
	}

	function GetListing($id, $params = array()) {
		return $this->return_first_result($this->MakeAPICall("GET", "listings/" . $id, '200m', $params));
	}

	function GetMyListings($params = array()) {
		return $this->return_all_results($this->MakeAPICall("GET", "my/listings", '20m', $params));
	}
